...
#define SNIMPL(res) if((res) < 0 && LOG_ALERT <= LOGLEVEL) { logstd(LOG_ALERT, __FILE__, __LINE__, #res); }
#define SNIMPL2(res, msg, ...) if((res) < 0 && LOG_ALERT <= LOGLEVEL) { logstd(LOG_ALERT, __FILE__, __LINE__, msg, ##__VA_ARGS__); }
